...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { label: 'Pending Reviews', value: stats?.pendingReviews || 0, color: 'text-red-400', icon: '⏳' },
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
        {statCards.map((s) => (
          <div key={s.label} className="bg-zinc-950 rounded-xl p-5 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { href: '/admin/reviews', label: 'Approve Reviews', desc: 'Manage pending reviews', icon: '✅', color: 'border-red-600' },
          { href: '/admin/users', label: 'Manage Users', desc: 'View and manage user roles', icon: '👥', color: 'border-green-500' },
        ].map((card) => (
          <Link key={card.href} href={card.href} className={`bg-zinc-950 border-l-4 ${card.color} rounded-xl p-6 hover:bg-zinc-900 transition`}>
