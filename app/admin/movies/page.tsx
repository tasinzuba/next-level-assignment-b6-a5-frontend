    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      {showForm && (
        <div className="bg-gray-900 rounded-xl p-6 mb-8">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex gap-6 items-center">
              <div>
                ...
                <select
                  ...
                  className="bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-2.5"
                >
            ...
            <div className="md:col-span-2">
              <button type="submit" disabled={saving} className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-6 py-2.5 rounded-lg transition disabled:opacity-50">
      ...
      {loading ? (
        <div className="animate-pulse space-y-3">
          {Array.from({ length: 5 }).map((_, i) => <div key={i} className="bg-gray-800 h-16 rounded" />)}
        </div>
      ) : (
        <div className="bg-gray-900 rounded-xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {movies.map((movie) => (
                <tr key={movie.id} className="border-t border-gray-800 hover:bg-gray-800/50 transition">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className={`text-xs px-2 py-1 rounded font-bold ${movie.priceType === 'PREMIUM' ? 'bg-yellow-500 text-black' : 'bg-green-700 text-white'}`}>
                      {movie.priceType}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-yellow-400 hidden md:table-cell">⭐ {movie.averageRating?.toFixed(1) || '-'}</td>
