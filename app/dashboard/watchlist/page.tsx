...
export default function WatchlistPage() {
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="bg-gray-200 dark:bg-zinc-900 rounded-lg aspect-[2/3] animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-8">My Watchlist</h1>

      {watchlist.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-gray-500 dark:text-gray-400 text-xl mb-4">Your watchlist is empty.</p>
            ...
            <div key={item.id} className="group relative">
              <Link href={`/movies/${item.movie.id}`}>
                <div className="bg-gray-100 dark:bg-zinc-900 rounded-lg overflow-hidden">
                  <div className="aspect-[2/3] bg-gray-200 dark:bg-zinc-800 relative">
                  ...
                  <div className="p-3">
                    <h3 className="font-semibold text-gray-900 dark:text-white text-sm truncate">{item.movie.title}</h3>
                    <p className="text-gray-500 dark:text-gray-400 text-xs mt-1">{item.movie.releaseYear}</p>
                  </div>
                </div>
              </Link>
              <button
