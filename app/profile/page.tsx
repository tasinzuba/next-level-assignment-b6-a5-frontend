...
export default function ProfilePage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-10">
        <div className="animate-pulse space-y-4">
          <div className="bg-gray-800 h-12 rounded" />
          <div className="bg-gray-800 h-48 rounded" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 space-y-8">
      ...
      {/* Stats Card */}
      {profile && (
        <div className="bg-gray-900 rounded-xl p-6 flex flex-col md:flex-row gap-6 items-center">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold ${profile.role === 'ADMIN' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300'}`}>
              {profile.role}
            </span>
          </div>
          <div className="flex gap-6 text-center">
            <div>
              <p className="text-2xl font-bold text-yellow-400">{profile._count?.reviews || 0}</p>
              <p className="text-gray-400 text-sm">Reviews</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-yellow-400">{profile._count?.watchlist || 0}</p>
      ...
      <div className="bg-gray-900 rounded-xl p-6">
        ...
        <form onSubmit={handleUpdateName} className="space-y-4">
          ...
          <button type="submit" disabled={saving} className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-6 py-2 rounded-lg transition disabled:opacity-50">
